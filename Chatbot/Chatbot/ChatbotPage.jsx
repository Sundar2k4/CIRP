...
const ChatbotPage = () => {
  ...
  const handleNewChat = () => {
    saveCurrentConversation();
    const newConversation = { name: `Chat ${conversations.length + 1}`, messages: [] };
  ...
  const handleDeleteConversation = (index) => {
    if (confirm('Are you sure you want to delete this conversation?')) {
      ...
      if (currentConversationIndex === index) {
        ...
        setCurrentConversation(updatedConversations.length ? updatedConversations[0].messages : []);
      }
    }
  };
    ...
    <div className="chatbot-page">
      <div className="welcome-banner">
        <div className="title-container">
          <span className="welcome-text">Welcome, <span className="username">User</span>!</span>
        ...
        <div className="sidebar">
          ...
          <div className="conversation-list">
            {conversations.map((conv, index) => (
              <div
                key={index}
                className={`chat-button ${index === currentConversationIndex ? 'active-chat' : ''}`}
                onClick={() => {
              ...
              >
                ...
                <div className="icon-container">
                  <span className="edit-icon" onClick={(e) => { e.stopPropagation(); setRenameModalOpen(true); setRenameInput(conv.name); }}>&#9998;</span>
                  <span className="delete-icon" onClick={(e) => { e.stopPropagation(); handleDeleteConversation(index); }}>&#128465;</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {renameModalOpen && (
        <div className="modal">
          ...
          />
          <button onClick={() => handleRenameConversation(currentConversationIndex)}>OK</button>
        </div>
      )}
    </div>
  );
